...
class ForkManage:
    def __init__(self, base_url: str, selector_list: List[str], logger):
    ...
    def fork_child(self, child_link: ChildLink, selector_list: List[str], level: int, exclude_link_url: Set[str],
                   fork_handler):
        if level < 0:
            return
        else:
        ...
        if not exclude_link_url.__contains__(child_url):
            ...
            response = Fork(child_link.url, selector_list, self.logger).fork()
...
class Fork:
    ...
    def __init__(self, base_fork_url: str, selector_list: List[str], logger):
    ...
    def get_content_html(self, bf: BeautifulSoup, remove_image: bool = True):
        if remove_image:
            for img in bf.body.find_all('img'):
                img.decompose()
    ...
    def fork(self, pure_res: bool = False):
        try:
            ...
            self.logger.info(f'fork:{self.base_fork_url}')
            response = httpx.get(self.base_fork_url, verify=False, headers=headers)
            if response.status_code != 200:
                self.logger.error(f"url: {self.base_fork_url} code:{response.status_code}")
                return Fork.Response.error(f"url: {self.base_fork_url} code:{response.status_code}")
            bf = self.get_beautiful_soup(response)
        except Exception as e:
            self.logger.error(f'{str(e)}:{traceback.format_exc()}')
...
def example_handler(base_url, response: Fork.Response):
    print(base_url.url, base_url.tag.text if base_url.tag else None, response.content)


def split_text_by_urls(text: str) -> List[Dict]:
    # Updated regex to match more URL formats and handle newlines
    # url_pattern = r'(\[.*?\]\(https?://[^\s<>_.-]+(\.[^\s<>_.-]+)*\)|<https?://[^\s<>_.-]+(\.[^\s<>_.-]+)*>)'
    url_pattern_with_newlines = r'(\[.*?\]\(https?://[\s\S]*?\)|<https?://[\s\S]*?>)'
    
    # Find all URLs and their positions
    matches = [(m.group(), m.start()) for m in re.finditer(url_pattern_with_newlines, text, re.DOTALL)]
    
    # List to hold the split elements with labels
    split_elements = []
    last_index = 0

    # Split text by URLs
    for url, start_index in matches:
        # Add text before the URL
        if last_index < start_index:
            split_elements.append({'type': 'text', 'content': text[last_index:start_index]})
        # Add the URL itself
        split_elements.append({'type': 'url', 'content': url})
        # Update last index
        last_index = start_index + len(url)
    
    # Add any remaining text after the last URL
    if last_index < len(text):
        split_elements.append({'type': 'text', 'content': text[last_index:]})
    
    return split_elements

def split_by_fixed_length_optimized(text, length):
    elements = split_text_by_urls(text)
    chunks = []
    current_chunk = []
    current_length = 0
    ...
    for element in elements:
        ...
        if element['type'] == 'text':
            while element_length > 0:
                if current_length + element_length > length:
                    remaining_text = element['content'][length - current_length:]
                    element['content'] = element['content'][:length - current_length]
                    if current_chunk:
                        chunks.append(current_chunk)
                    current_chunk = [element['content']]
                    element = {'type': 'text', 'content': remaining_text}
                    current_length = len(remaining_text)
                    element_length = len(remaining_text)
                else:
